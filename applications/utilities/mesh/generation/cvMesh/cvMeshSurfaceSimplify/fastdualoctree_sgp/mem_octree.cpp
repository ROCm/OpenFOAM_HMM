...
void MemOctree::compute_opt_level()
//-----------------------------------------------------------------------------
{
  Level left     = 0 ;
...
//-----------------------------------------------------------------------------
{
  ...
  for( cell_iterator it = cells_begin() ; it() ; ++it )
  {
    ...
    {
      printf("(mem) Invalid leaf %d: value %f\n", (int)k, _hash[k].data ) ;
      result = false ;
    }

    if( has_son && !all_sons )
    {
      printf("(mem) Invalid subdivision %d: value %f\n", (int)k, _hash[k].data ) ;
      result = false ;
    }
  }
...
bool MemOctree::set_impl( data_access *ref )
//-----------------------------------------------------------------------------
{
  if( !ref ) return false ;
...
bool MemOctree::refine( data_access *ref /*= NULL*/ )
//-----------------------------------------------------------------------------
{
  if( !ref ) return false ;
...
//_____________________________________________________________________________
// Refine the octree according to the data access
bool MemOctree::adapt( data_access *ref /*= NULL*/ )
//-----------------------------------------------------------------------------
{
  if( !ref ) return false ;
...
//_____________________________________________________________________________
// Octree dual caller
bool MemOctree::dual_cubes_walk( mem_dual_walker &walker )
//-----------------------------------------------------------------------------
{
  bool result = true ;
  ...
  for( leaf_iterator l_it = leaves_begin() ; l_it() ; ++l_it )
  {
    ...
    for( int i = 0 ; i < 8 ; ++i, ++v_ptr )
    {
      ...
      Key dual_verts[8] ;
      dual_vertices_keys( k, lv, dual_verts ) ;
      
      bool run_walker = true ;
      Key *dv_ptr = dual_verts ;
      for( int j = 0 ; j < 8 ; ++j, ++dv_ptr )
      {
        if( j == i ) continue ;
        
        HashField::KeyData kd ;
        
        // neighbor has bigger size: skip
        //   must be processed by the deepest level
        if( !node_exists(*dv_ptr,kd) )
          continue ;
        
        // neighbor is deeper: break
        //   will be processed by that neighbor
        if( !is_leaf(kd) )
        {
          run_walker = false ;
          break ;
        ...
        // neighbor has same level
        //   choose the first one in the order
        if( j < i )
        {
          run_walker = false ;
          break ;
...
//-----------------------------------------------------------------------------
{
  ...
  for( int j = 0 ; j < 8 ; ++j )
  {
    ...
    const Key k = keys[j] ;
    MemOctree::geom_cell c = fo.geom_key(k) ;
    cube[i] = *c ;
...
bool MemOctree::build_isosurface( data_access *ref )
//-----------------------------------------------------------------------------
{
  if( !ref ) return false ;
  _mc.dat_access() = ref ;
  ...
  result = dual_cubes_walk( mem_isosurface_walker ) ;
...
//-----------------------------------------------------------------------------
{
  ...
  {
    int i = (j==2)?3:(j==3)?2:(j==7)?6:(j==6)?7:j ;
    const MemOctree::geom_cell c = fo.geom_key( keys[j] ) ;
    cube[i] = *c ;
...
//_____________________________________________________________________________
// Build the isosurface using dual marching cubes
bool MemOctree::direct_draw_isosurface( data_access *ref )
//-----------------------------------------------------------------------------
{
  if( !ref ) return false ;
  ...
  {
    result = dual_cubes_walk( mem_direct_isosurface_walker ) ;
  }
  ::glEnd() ; // GL_TRIANGLES
...
bool MemOctree::draw_dual()
//-----------------------------------------------------------------------------
{
  ::glBegin( GL_LINES ) ;
  bool result = dual_cubes_walk( mem_draw_walker ) ;
  ::glEnd() ;  // GL_LINES
...
bool MemOctree::dual_timing()
//-----------------------------------------------------------------------------
{
    return dual_cubes_walk( mem_timing_walker ) ;
}
//_____________________________________________________________________________
...
bool MemOctree::find_leaf( Key k, Level o_lv, geom_cell &n ) const
//-----------------------------------------------------------------------------
{
...
bool MemOctree::find_radius( real x, real y, real z, real r, List<geom_cell> &cells ) const
//-----------------------------------------------------------------------------
{
...
bool MemOctree::adjacent( Key k, List<geom_cell> &cells ) const
//-----------------------------------------------------------------------------
{
...
void MemOctree::draw_plane ( real nx, real ny, real nz, real d )
//-----------------------------------------------------------------------------
{
...
void MemOctree::draw_slice ( real nx, real ny, real nz, real d, float alpha )
//-----------------------------------------------------------------------------
{
  geom_cell n = geom_root() ;
