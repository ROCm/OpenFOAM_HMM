...
/** Write modification time in ADF_Close, otherwise NFS performance is
...
CHECK_ADF_ABORT( *error_return ) ;
	...
	/** Write modification time in ADF_Database_Close but nowhere else,
	    otherwise NFS performance is even worse than normal */
ADFI_write_modification_date( file_index, error_return ) ;
CHECK_ADF_ABORT( *error_return ) ;
...
/** Write modification time in ADF_Database_Close, otherwise NFS performance is
    even worse than normal
ADFI_write_modification_date( file_index, error_return ) ;
CHECK_ADF_ABORT( *error_return ) ; */
...
char        file_format, machine_format ;
int         file_bytes, memory_bytes ;
long        total_bytes, bytes_written, bytes_to_write ;
int         i, j ;
char        tag[TAG_SIZE+1] ;
struct DISK_POINTER     data_start, chunk_start, end_of_chunk_tag ;
long        start_offset ;
