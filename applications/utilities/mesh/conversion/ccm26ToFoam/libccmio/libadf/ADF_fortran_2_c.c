...
for( i=0; i<*inum_ret; i++ )   { /* Blank-fill the names */
   if( strlen( &tmp_names[ i * (*name_length+1) ]) == (unsigned)*name_length) {
...
if( *error_return == NO_ERROR ) {
   if( strlen( ctype ) < (unsigned)*data_type_length ) {
...
if( *error_return == NO_ERROR ) {
   if( strlen( clabel ) < (unsigned)*label_length ) {
...
if( *error_return == NO_ERROR ) {
   ...
   if( strlen(cpath) < (unsigned)*link_path_length ) {
...
if( *error_return == NO_ERROR ) {
   if( strlen( cname ) < (unsigned)*name_length ) {
