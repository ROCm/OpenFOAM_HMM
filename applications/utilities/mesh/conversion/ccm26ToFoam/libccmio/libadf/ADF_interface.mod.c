...
extern void tStart(Timer *t);
extern void tEnd(Timer *t);

extern Timer gADF_Create_Node_Timer;
extern Timer gADF_Create_Node_Prep_Timer;
extern Timer gADF_Create_Node_Write_Timer;
extern Timer gADF_Create_Node_After_Timer;
extern Timer gADF_Create_Node_With_Label_Timer;
extern Timer gADF_Create_Node_With_Label_Prep_Timer;
extern Timer gADF_Create_Node_With_Label_Write_Timer;
extern Timer gADF_Create_Node_With_Label_After_Timer;
extern Timer gADF_Children_Names_Timer;
extern Timer gADF_Children_IDs_Timer;
extern Timer gADF_Database_Close_Timer;
extern Timer gADF_Database_Get_Format_Timer;
extern Timer gADF_Database_Open_Timer;
extern Timer gADF_Database_Set_Format_Timer;
extern Timer gADF_Database_Version_Timer;
extern Timer gADF_Delete_Timer;
extern Timer gADF_Get_Data_Type_Timer;
extern Timer gADF_Get_Dimension_Values_Timer;
extern Timer gADF_Get_Label_Timer;
extern Timer gADF_Get_Link_Path_Timer;
extern Timer gADF_Get_Name_Timer;
extern Timer gADF_Get_Node_ID_Timer;
extern Timer gADF_Get_Number_of_Dimensions_Timer;
extern Timer gADF_Get_Root_ID_Timer;
extern Timer gADF_Is_Link_Timer;
extern Timer gADF_Number_of_Children_Timer;
extern Timer gADF_Put_Dimension_Information_Timer;
extern Timer gADF_Put_Name_Timer;
extern Timer gADF_Read_All_Data_Timer;
extern Timer gADF_Read_Block_Data_Timer;
extern Timer gADF_Read_Data_Timer;
extern Timer gADF_Set_Label_Timer;
extern Timer gADF_Write_All_Data_Timer;
extern Timer gADF_Write_Block_Data_Timer;
extern Timer gADF_Write_Data_Timer;
...
tStart(&gADF_Children_Names_Timer);
...
if( node.num_sub_nodes == 0 ) {
   tEnd(&gADF_Children_Names_Timer);
   return ;
   } /* end if */
...
tStart(&gADF_Children_IDs_Timer);
ADFI_chase_link( PID, &LID, &file_index,  &block_offset, &node, error_return ) ;
CHECK_ADF_ABORT( *error_return ) ;

   /** Check for zero children, return if 0 **/
if( node.num_sub_nodes == 0 ) {
   tEnd(&gADF_Children_IDs_Timer);
   return ;
   } /* end if */
...
printf("-------- ADF_Create(%s) ----------- \n", name);
 ...
 tStart(&gADF_Create_Node_Prep_Timer);
...
for ( i=0; i < name_length; i++ ) {
 ...
 tEnd(&gADF_Create_Node_Prep_Timer);

	/** Assign the name to the new node **/
tStart(&gADF_Create_Node_Write_Timer);
...
ADFI_file_malloc( file_index, NODE_HEADER_SIZE, &child_block_offset,
			  error_return ) ;
 printf("fmalloc child node: (%d, %d)\n", child_block_offset.block,
	child_block_offset.offset);
CHECK_ADF_ABORT( *error_return ) ;

	/** Update the list of children for the parent... **/
tStart(&gADF_Create_Node_After_Timer);
ADFI_add_2_sub_node_table( file_index, &parent_block_offset,
			   &child_block_offset, /*child_node.name,*/ error_return ) ;
CHECK_ADF_ABORT( *error_return ) ;

	/** Write out the new node header **/
ADFI_write_node_header( file_index, &child_block_offset, &child_node,
		error_return ) ;
printf("write child header: (%d, %d)\n", child_block_offset.block,
       child_block_offset.offset);
CHECK_ADF_ABORT( *error_return ) ;
tEnd(&gADF_Create_Node_Write_Timer);
...
CHECK_ADF_ABORT( *error_return ) ;
#endif
tEnd(&gADF_Create_Node_After_Timer);
tEnd(&gADF_Create_Node_Timer);
printf("-----------------------\n");
} /* end of ADF_Create */
/* end of file ADF_Create.c */

/***********************************************************************
ADF Create with Label:
...
ADF_Create_With_Label( PID, name, const char *label, ID, error_return )
input:  const double PID	The ID of the parent node, to whom we
				are creating a new child node.
input:  const char *name	The name of the new child.
input:  const char *label	The label of the new child.
output: double *ID		The ID of the newly created node.
output: int *error_return	Error return.
...
struct NODE_HEADER		parent_node, child_node ;
struct SUB_NODE_TABLE_ENTRY	sub_node_entry ;
int				i, name_length, label_length, name_start, found;
double				LID ;

tStart(&gADF_Create_Node_With_Label_Timer);
ADFI_check_string_length( name, ADF_NAME_LENGTH, error_return ) ;
CHECK_ADF_ABORT( *error_return ) ;
...
tStart(&gADF_Create_Node_With_Label_Prep_Timer);
ADFI_chase_link( PID, &LID, &file_index,  &parent_block_offset,
		&parent_node, error_return ) ;
CHECK_ADF_ABORT( *error_return ) ;
...
for ( i=0; i < name_length; i++ ) {
 ...
 tEnd(&gADF_Create_Node_Prep_Timer);
	...
	/** Assign the name to the new node **/
tStart(&gADF_Create_Node_With_Label_Write_Timer);
strncpy( child_node.name, &name[ name_start ], name_length ) ;
...
ADFI_write_node_header( file_index, &child_block_offset, &child_node,
		error_return ) ;
CHECK_ADF_ABORT( *error_return ) ;
tEnd(&gADF_Create_Node_With_Label_Write_Timer);

	/** OK, new node is on disk.  Now, update the list of
	    children for the parent...
	**/
tStart(&gADF_Create_Node_With_Label_After_Timer);
ADFI_add_2_sub_node_table( file_index, &parent_block_offset,
			   &child_block_offset, /*name,*/ error_return ) ;
CHECK_ADF_ABORT( *error_return ) ;
...
tEnd(&gADF_Create_Node_With_Label_After_Timer);
tEnd(&gADF_Create_Node_With_Label_Timer);
}
...
tStart(&gADF_Database_Close_Timer);
...
CHECK_ADF_ABORT( *error_return ) ;
tEnd(&gADF_Database_Close_Timer);
...
tStart(&gADF_Database_Get_Format_Timer);
	/** Get the file, block, and offset numbers from the ID **/
ADFI_ID_2_file_block_offset( Root_ID, &file_index, &block_offset.block,
		&block_offset.offset, error_return ) ;
CHECK_ADF_ABORT( *error_return ) ;
...
tEnd(&gADF_Database_Get_Format_Timer);
...
tStart(&gADF_Database_Open_Timer);
*error_return = NO_ERROR ;
...
tStart(&gADF_Database_Set_Format_Timer);
...
tEnd(&gADF_Database_Set_Format_Timer);
...
tStart(&gADF_Database_Version_Timer);
	/** Get the file, block, and offset numbers from the ID **/
ADFI_ID_2_file_block_offset( Root_ID, &file_index, &block_offset.block,
		&block_offset.offset, error_return ) ;
CHECK_ADF_ABORT( *error_return ) ;
...
tEnd(&gADF_Database_Version_Timer);
...
if( link_path_length > 0 ) { /** this node IS a link **/
   ...
   for( i=0; i<num_ids; i++ ) {
      tEnd(&gADF_Delete_Timer);
      ADF_Delete( ID, ids[i], error_return ) ;  /* resursion */
      gADF_Delete_Timer.iterations--;
      tStart(&gADF_Delete_Timer);
      CHECK_ADF_ABORT( *error_return ) ;
      } /* end for */
...
tStart(&gADF_Get_Dimension_Values_Timer);
*error_return = NO_ERROR ;
...
tEnd(&gADF_Get_Dimension_Values_Timer);
...
tStart(&gADF_Get_Link_Path_Timer);
        /** Get the file, block, and offset numbers from the ID **/
...
tStart(&gADF_Get_Number_of_Dimensions_Timer);
*error_return = NO_ERROR ;
...
tEnd(&gADF_Get_Number_of_Dimensions_Timer);
...
ADFI_add_2_sub_node_table( file_index, &new_parent, &child, /*child_name,*/
			   error_return ) ;
CHECK_ADF_ABORT( *error_return ) ;
...
tStart(&gADF_Number_of_Children_Timer);
*error_return = NO_ERROR ;
...
tStart(&gADF_Put_Dimension_Information_Timer);
...
tEnd(&gADF_Put_Dimension_Information_Timer);
...
tStart(&gADF_Read_All_Data_Timer);
*error_return = NO_ERROR ;
...
tStart(&gADF_Read_Block_Data_Timer);
*error_return = NO_ERROR ;
...
if( node.number_of_data_chunks == 0  ) {
   ...
   *error_return = NO_DATA ;
   tEnd(&gADF_Read_Block_Data_Timer);
   return ;	/** NO_DATA is really a warning, so don't check & abort... **/
   } /* end if */
...
tStart(&gADF_Write_All_Data_Timer);
*error_return = NO_ERROR ;
...
tEnd(&gADF_Write_All_Data_Timer);
...
tStart(&gADF_Write_Block_Data_Timer);
*error_return = NO_ERROR ;
...
tEnd(&gADF_Write_Block_Data_Timer);
