...
temp.block = block_offset->block ;
	...
	/** Do this incrementally to avoid numerical oddities... **/
while( block_offset->offset >= DISK_BLOCK_SIZE ) {
   ...
   temp.block = block_offset->block ;
   block_offset->offset = block_offset->offset - DISK_BLOCK_SIZE ;
   } /* end while */
...
input:  const int dim_stride[]	The stride to take in our sub-space
				(every Nth element)
output: int *total_points	Total points defined in our sub-space.
output: int *starting_offset	Number of elements skipped before first element
output: int *error_return	Error return.
...
output: int *element_offset	Number of elements to jump to next (1 to N)
output: int *error_return	Error return.
...
void	ADFI_increment_array(
		...
		int *error_return )
{
unsigned int		i, offset, accumlated_size ;
...
{
printf("Subroutine ADFI_is_block_in_core is not yet implemented...\n" ) ;
}
...
{
printf("Subroutine ADFI_read_disk_block is not yet implemented...\n" ) ;
...
{
printf("Subroutine ADFI_write_disk_block is not yet implemented...\n" ) ;
