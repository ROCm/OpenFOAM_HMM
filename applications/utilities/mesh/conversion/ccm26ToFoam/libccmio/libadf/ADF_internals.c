...
int DC_flush()
{
    ...
    for (i = 0;  i < n;  ++i) {
	if (fseeko(ADF_file[dirty_buffers[i]->index],
...
if( data_length + block_offset > DISK_BLOCK_SIZE ) {
    ...
    long start_pos = file_block * DISK_BLOCK_SIZE + block_offset;
    off_t end_pos = start_pos + data_length;
    unsigned long ending_block = end_pos / DISK_BLOCK_SIZE;

    if (file_block + 1 <= ending_block - 1)
	DC_flush_range(file_index, file_block + 1, ending_block - 1);
