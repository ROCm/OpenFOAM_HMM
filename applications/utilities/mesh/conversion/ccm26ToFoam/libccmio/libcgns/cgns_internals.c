...
int cgi_read_base(cgns_base *base) {
    ...
    /* update version */
    if (cg->mode == MODE_MODIFY && cg->version < 1100) {
        int ierr;
        dim_vals[0] = 2;
...
int cgi_read_zone(cgns_zone *zone) {
    ...
    /* update version */
    if (cg->mode == MODE_MODIFY && cg->version < 1100 && !in_link) {
        int ierr;
    ...
    if (fam_flag==1) {
        ...
        if (cg->version>1200) {
         ...
         /* FamilyName is the ADF node name */
            ...
            /* update version */
            if (cg->mode == MODE_MODIFY && !in_link) {
                double dummy_id;
         ...
         /* ElementConnectivity */
            if (strcmp(temp_name,"ElementConnectivity")==0) {
                 ...
                 /* Rewrite with new data array parameters starting with version 1200 */
                    ...
                    if (cg->mode == MODE_MODIFY && !linked) {
                        ...
                        ADF_Write_All_Data(section[0][n].connect->id,
                            (const char *) section[0][n].connect->data, &ierr);
                        if (ierr > 0) {
                            adf_error("ADF_Write_All_Data", ierr);
                            return 1;
                        }
             ...
             /* check data */
                ...
                if (section[0][n].parent->dim_vals[0] != nelements ||
...
int cgi_read_conn(cgns_conn *conn) {
    ...
    if (conn->location != Vertex && conn->location != CellCenter &&
        ...
        cgi_error("Unsupported GridLocation %s for Connectivity %s",
            conn->location < 0 || conn->location >= NofValidGridLocation ?
            "<invalid>" : GridLocationName[conn->location], conn->name);
        return 1;
    }
...
int cgi_read_boco(cgns_boco *boco) {
    ...
    for (n=0; n<nIA_t; n++) {
        ...
        boco->location = GridLocationNull;
        boco->ptset->id = IA_id[0];
        boco->ptset->link = cgi_read_link(IA_id[0]);
        boco->ptset->in_link = linked;
        if (cgi_read_ptset(boco->id, boco->ptset)) return 1;
    }
        ...
        if (boco->dataset[n].dirichlet) {
            for (i=0; i<boco->dataset[n].dirichlet->narrays; i++) {
                ...
                if (array.data_dim!=1 || (array.dim_vals[0] != 1 &&
                    array.dim_vals[0] != boco->ptset->size_of_patch)) {
    ...
    if (cg->version <= 1270) {
        ...
        if (boco->ptset->type == ElementRange ||
            boco->ptset->type == ElementList) {
            if (boco->ptset->type == ElementRange)
                boco->ptset->type = PointRange;
            else
                boco->ptset->type = PointList;
            boco->location = FaceCenter;
            if (cg->mode == MODE_MODIFY && !linked) {
                name = PointSetTypeName[boco->ptset->type];
                ADF_Put_Name(boco->id, boco->ptset->id, name, &ierr);
                if (ierr > 0) {
...
int cgi_read_ptset(double parent_id, cgns_ptset *ptset) {
    ...
    if (cg->version <= 1200 && ndim == 1 &&
       (ptset->type == ElementRange || ptset->type == ElementList)) {
        ...
        dim_vals[0]=Idim;
        if (cg->mode == MODE_MODIFY && ptset->link == 0 &&
            ptset->in_link == 0) {
            int ierr;
    ...
    if (ptset->type == PointList || ptset->type == ElementList ||
     ...
     /* read points to calculate size_of_patch */
        ...
        ADF_Read_All_Data(ptset->id, (char *) pnts, &ierr);
        if (ierr>0) {
...
int cgi_read_exponents(int in_link, double parent_id, cgns_exponent **exponents) {
    ...
    if (cgi_read_node(exponents[0]->id, exponents[0]->name, exponents[0]->data_type,
        &ndim, dim_vals, &exponents[0]->data, READ_DATA)) {
        cgi_error("Error reading '%s'",exponents[0]->name);
        return 1;
    }
    if (strcmp(exponents[0]->data_type,"R4") && strcmp(exponents[0]->data_type,"R8")) {
...
/* The following was replaced by a 5 element array:  exponents[0]->data
    if (strcmp(data_type,"R4")==0) {
        *((float*)exponents[0]->       MassExponent)=*((float*)data+0);
        *((float*)exponents[0]->     LengthExponent)=*((float*)data+1);
        *((float*)exponents[0]->       TimeExponent)=*((float*)data+2);
        *((float*)exponents[0]->TemperatureExponent)=*((float*)data+3);
        *((float*)exponents[0]->      AngleExponent)=*((float*)data+4);
    } else {
        *((double*)exponents[0]->       MassExponent)=*((double*)data+0);
        *((double*)exponents[0]->     LengthExponent)=*((double*)data+1);
        *((double*)exponents[0]->       TimeExponent)=*((double*)data+2);
        *((double*)exponents[0]->TemperatureExponent)=*((double*)data+3);
        *((double*)exponents[0]->      AngleExponent)=*((double*)data+4);
    }
*/
    return 0;
}
...
int cgi_read_units(int in_link, double parent_id, cgns_units **units) {
    ...
    char_33 mass_name, len_name, time_name, temp_name, angle_name;
    char *string_data;
    double *id;
    int n, nnod;
    ...
    if (strlen(string_data) != 32*5) {
     ...
     /* Extract Units */
    for (n=0; n<32; n++) {
        mass_name[n]=string_data[n];
        len_name[n]=string_data[n+32];
        time_name[n]=string_data[n+64];
        temp_name[n]=string_data[n+96];
        angle_name[n]=string_data[n+128];
    }
    mass_name[32]=len_name[32]=time_name[32]=temp_name[32]=angle_name[32]='\0';

    cgi_MassUnits(mass_name, &units[0]->mass);
    cgi_LengthUnits(len_name, &units[0]->length);
    cgi_TimeUnits(time_name, &units[0]->time);
    cgi_TemperatureUnits(temp_name, &units[0]->temperature);
    cgi_AngleUnits(angle_name, &units[0]->angle);
...
int cgi_read_rind(double parent_id, int **rind_planes) {
    ...
    if (CurrentZoneType == Unstructured && (rind_planes[0]!=0 || rind_planes[1]!=0)) {
        cgi_error("Rind planes are not supported for unstructured meshes.");
        return 1;
    }
    free(id);
    return 0;
...
int cgi_read_node(double node_id, char_33 name, char_33 data_type,
    ...
    if (strcmp(data_type,"I4")==0) data[0]=CGNS_NEW(int, size);
     ...
     /* read data */
    ADF_Read_All_Data(node_id, (char *) data[0], &ierr);
    if (ierr>0) {
        adf_error("ADF_Read_All_Data",ierr);
        return 1;
    }
...
int cgi_write_ptset(double parent_id, char_33 name, cgns_ptset *ptset,
    ...
    if (ptset->link) {
     ...
     /* Set label */
#if 0    /* re-enable ElementList/ElementRange */
    if (ptset->type == PointRange || ptset->type == PointRangeDonor)
#else
...
int cgi_write_array(double parent_id, cgns_array *array) {
    ...
    if (array->exponents) {
        dim_vals=5;
        if (cgi_new_node(array->id, "DimensionalExponents",
            "DimensionalExponents_t", &array->exponents->id,
            array->exponents->data_type, 1, &dim_vals, array->exponents->data))
            return 1;
    }
     /* DataConversion_t */
    if (array->convert) {
...
int cgi_new_node(double parent_id, char const *name, char const *label,
    ...
    ADF_Write_All_Data(*node_id, (const char *) data, &ierr);
    if (ierr>0) {
        adf_error("ADF_Write_All_Data", ierr);
        return 1;
    }
...
#if 0
    for (nid = 0, n = 1; n <= nchildren; n++) {
        ADF_Children_IDs (parent_id, n, 1, &len, &nodeid, &ierr);
        if (ierr > 0) {
            adf_error ("ADF_Children_IDs", ierr);
            return 1;
        }
        ADF_Get_Label (nodeid, nodelabel, &ierr);
        if (ierr > 0) {
...
int cgi_check_mode(char const *filename, int file_mode, int mode_wanted) {
    ...
    if (mode_wanted==MODE_READ && file_mode==MODE_WRITE) {
        cgi_error("File %s not open for reading", filename);
        return 1;
    }
    if (mode_wanted==MODE_WRITE && file_mode==MODE_READ) {
...
cgns_file *cgi_get_file(int file_number) {
    ...
    if (file_number <= 0 || file_number > n_cgns_files) {
        cgi_error("CGNS file %d is not open",file_number);
        return 0;
    }
    cg = &(cgns_files[file_number-1]);
    if (cg->mode == MODE_CLOSED) {
...
cgns_zcoor *cgi_get_zcoorGC(cgns_file *cg, int B, int Z) {
    ...
    if (zone->nzcoor == 0 && (cg->mode == MODE_WRITE || cg->mode == MODE_MODIFY)) {
...
static void *cgi_get_darray_posit(cgns_array *array, int narrays, int* index, int n, int m) {

    if (array == 0)
        return 0;
    else if (! (index[m] <= narrays && index[m] > 0))
        cgi_error("DataArray_t node number %d invalid",index[m]);
    else if ((n-1) == m)
        return (void *)&array[index[m]-1];

    return 0;
}

static void *cgi_get_user_data_posit(cgns_user_data *user_data, int nuser_data,
        int* index, int n, int m) {

    if (user_data == 0)
        return 0;
    else if (! (index[m] <= nuser_data && index[m] > 0))
        cgi_error("UserDefinedData_t node number %d invalid",index[m]);
    else if ((n-1) == m)
        return (void *)&user_data[index[m]-1];
    else
        return cgi_get_darray_posit(user_data[index[m]-1].array,
            user_data[index[m]-1].narrays, index, n, m+1);

    return 0;
}

void *cgi_get_posit(int fn, int B, int n, int *index, char **label, int *ier) {
/* This function returns the memory address pointed by the cg_goto arguments.       *\
\* All nodes that can be reached with cg_goto must be represented here.         */
    int i, j;
    int Z;
#define n_valid_labels 42
    static char *valid_labels[n_valid_labels] = {
        "Zone_t",
        "GridCoordinates_t",
        "DataArray_t",
        "FlowSolution_t",
    ...
    /* check input labels */
    for (j=0; j<n; j++) {
        for (i=0; i<n_valid_labels && strcmp(label[j],valid_labels[i]); i++);
        if (i==n_valid_labels) {
            cgi_error("The label prescribed '%s' is not supported by the function cg_goto", label[j]);
            (*ier)=CG_ERROR;
            return 0;
        }
    }
    ...
    if (n == 0)  /* No labels in goto path.  Must be CGNSBase_t node. */
        posit = (void *) cgi_get_base(cg, B);
    else if ((n > 0) && (strcmp(label[0], "Zone_t") == 0)) {
        Z = index[0];
        if (n == 1) {
            cgns_zone *zone = cgi_get_zone(cg, B, Z);
            posit = (void *)zone;
        }
        /* Process children of Zone_t. */
        else if ((n > 1) && (strcmp(label[1], "GridCoordinates_t") == 0)) {
            cgns_zcoor *zcoor = cgi_get_zcoor(cg, B, Z, index[1]);
            if (n == 2)
                posit = (void *)zcoor;
            else if (zcoor && (n > 2) && (strcmp(label[2], "DataArray_t") == 0))
                posit = cgi_get_darray_posit(zcoor->coord, zcoor->ncoords, index, n, 2);
            else if (zcoor && (n > 2) && (strcmp(label[2], "UserDefinedData_t") == 0) )
                posit = cgi_get_user_data_posit(zcoor->user_data, zcoor->nuser_data, index, n, 2);

        } else if ((n > 1) && (strcmp(label[1], "ZoneIterativeData_t") == 0)) {
            cgns_ziter *ziter = cgi_get_ziter(cg, B, Z);
            if (n == 2)
                posit = (void *)ziter;
            else if (ziter && (n > 2) && (strcmp(label[2], "DataArray_t") == 0) )
                posit = cgi_get_darray_posit(ziter->array, ziter->narrays, index, n, 2);
            else if (ziter && (n > 2) && (strcmp(label[2], "UserDefinedData_t") == 0) )
                posit = cgi_get_user_data_posit(ziter->user_data, ziter->nuser_data, index, n, 2);

        } else if ((n > 1) && (strcmp(label[1], "Elements_t") == 0)) {
            cgns_zone *zone = cgi_get_zone(cg, B, Z);
            cgns_section *section = 0;

            /* check index */
            if (zone && index[1] <= zone->nsections && index[1] > 0)
                section = &zone->section[index[1]-1];
            else if (zone)
                cgi_error("Elements_t node number %d invalid",index[1]);

            if (n == 2)
                posit = (void *) section;
            else if (section && (n > 2) && (strcmp(label[2], "UserDefinedData_t") == 0) )
                posit = cgi_get_user_data_posit(section->user_data, section->nuser_data, index, n, 2);

        } else if ((n > 1) && (strcmp(label[1], "FlowSolution_t") == 0)) {
            cgns_sol *sol = cgi_get_sol(cg, B, Z, index[1]);
            if (n == 2)
                posit = (void *)sol;
            else if (sol && (n > 2) && (strcmp(label[2], "DataArray_t") == 0) )
                posit = cgi_get_darray_posit(sol->field, sol->nfields, index, n, 2);
            else if (sol && (n > 2) && (strcmp(label[2], "UserDefinedData_t") == 0) )
                posit = cgi_get_user_data_posit(sol->user_data, sol->nuser_data, index, n, 2);

        } else if ((n > 1) && (strcmp(label[1], "RigidGridMotion_t") == 0)) {
            cgns_rmotion *rmotion = cgi_get_rmotion(cg, B, Z, index[1]);
            if (n == 2)
                posit = (void *)rmotion;
            else if (rmotion && (n > 2) && (strcmp(label[2], "DataArray_t") == 0) )
                posit = cgi_get_darray_posit(rmotion->array, rmotion->narrays, index, n, 2);
            else if (rmotion && (n > 2) && (strcmp(label[2], "UserDefinedData_t") == 0) )
                posit = cgi_get_user_data_posit(rmotion->user_data, rmotion->nuser_data, index, n, 2);

        } else if ((n > 1) && (strcmp(label[1], "ArbitraryGridMotion_t") == 0)) {
            cgns_amotion *amotion = cgi_get_amotion(cg, B, Z, index[1]);
            if (n == 2)
                posit = (void *)amotion;
            else if (amotion && (n > 2) && (strcmp(label[2], "DataArray_t") == 0) )
                posit = cgi_get_darray_posit(amotion->array, amotion->narrays, index, n, 2);
            else if (amotion && (n > 2) && (strcmp(label[2], "UserDefinedData_t") == 0) )
                posit = cgi_get_user_data_posit(amotion->user_data, amotion->nuser_data, index, n, 2);

        } else if ((n > 1) && (strcmp(label[1], "ZoneGridConnectivity_t") == 0)) {
            cgns_zconn *zconn = cgi_get_zconn(cg, B, Z);
            if (n == 2)
                posit = (void *)zconn;
            else if (zconn && (n > 2) && (strcmp(label[2], "OversetHoles_t") == 0) ) {
                cgns_hole *hole = cgi_get_hole(cg, B, Z, index[2]);
                if (n == 3)
                    posit = (void *)hole;
                else if (hole && (n > 3) && (strcmp(label[3], "UserDefinedData_t") == 0) )
                    posit = cgi_get_user_data_posit(hole->user_data, hole->nuser_data, index, n, 3);

            } else if (zconn && (n > 2) && (strcmp(label[2], "GridConnectivity_t") == 0) ) {
                cgns_conn *conn = cgi_get_conn(cg, B, Z, index[2]);
                if (n == 3)
                    posit = (void *)conn;
                else if (conn && (n > 3) && (strcmp(label[3], "UserDefinedData_t") == 0) )
                    posit = cgi_get_user_data_posit(conn->user_data, conn->nuser_data, index, n, 3);
                else if (conn && (n > 3) && (strcmp(label[3], "GridConnectivityProperty_t") == 0) ) {
                    cgns_cprop *cprop = cgi_get_cprop(cg, B, Z, index[2]);
                    if (n == 4)
                        posit = (void *) cprop;
                    else if (cprop && (n > 4) && (strcmp(label[4], "Periodic_t") == 0) ) {
                        cgns_cperio *cperio = cprop -> cperio;
                        if (n == 5)
                            posit = (void *) cperio;
                        else if (cperio && (n > 5) && (strcmp(label[5], "UserDefinedData_t") == 0) )
                            posit = cgi_get_user_data_posit(cperio->user_data, cperio->nuser_data, index, n, 5);
                        else if (cperio && (n > 5) && (strcmp(label[5], "DataArray_t") == 0) )
                            posit = cgi_get_darray_posit(cperio->array, cperio->narrays, index, n, 5);
                    } else if (cprop && (n > 4) && (strcmp(label[4], "AverageInterface_t") == 0) ) {
                        cgns_caverage *caverage = cprop -> caverage;
                        if (n == 5)
                            posit = (void *) caverage;
                        else if (caverage && (n > 5) && (strcmp(label[5], "UserDefinedData_t") == 0) )
                            posit = cgi_get_user_data_posit(caverage->user_data, caverage->nuser_data, index, n, 5);
                    } else if (cprop && (n > 4) && (strcmp(label[4], "UserDefinedData_t") == 0) )
                        posit = cgi_get_user_data_posit(cprop->user_data, cprop->nuser_data, index, n, 4);
                }


            } else if (zconn && (n > 2) && (strcmp(label[2], "GridConnectivity1to1_t") == 0) ) {
                cgns_1to1 *conn1to1 = cgi_get_1to1(cg, B, Z, index[2]);
                if (n == 3)
                    posit = (void *)conn1to1;
                else if (conn1to1 && (n > 3) && (strcmp(label[3], "UserDefinedData_t") == 0) )
                    posit = cgi_get_user_data_posit(conn1to1->user_data, conn1to1->nuser_data, index, n, 3);

            } else if (zconn && (n > 2) && (strcmp(label[2], "UserDefinedData_t") == 0) )
                posit = cgi_get_user_data_posit(zconn->user_data, zconn->nuser_data, index, n, 2);

        } else if ((n > 1) && (strcmp(label[1], "ZoneBC_t") == 0)) {
            cgns_zboco *zboco = cgi_get_zboco(cg, B, Z);
            if (n == 2)
                posit = (void *)zboco;
            else if (zboco && (n > 2) && (strcmp(label[2], "BC_t") == 0) ) {
                cgns_boco *boco = cgi_get_boco(cg, B, Z, index[2]);
                if (n == 3)
                    posit = (void *)boco;
                else if (boco && (n > 3) && (strcmp(label[3], "BCDataSet_t") == 0) ) {
                    cgns_dataset *dataset = cgi_get_dataset(cg, B, Z, index[2], index[3]);
                    if (n == 4)
                        posit = (void *)dataset;
                    else if (dataset && (n > 4) && (strcmp(label[4], "BCData_t") == 0) ) {
                        cgns_bcdata *bcdata = cgi_get_bcdata(cg, B, Z, index[2], index[3],
                            (BCDataType_t)index[4]);
                        if (n == 5)
                            posit = (void *)bcdata;
                        else if (bcdata && (n > 5) && (strcmp(label[5], "DataArray_t") == 0) )
                            posit = cgi_get_darray_posit(bcdata->array, bcdata->narrays, index, n, 5);
                        else if (bcdata && (n > 5) && (strcmp(label[5], "UserDefinedData_t") == 0) )
                            posit = cgi_get_user_data_posit(bcdata->user_data, bcdata->nuser_data,
                                index, n, 5);

                    } else if (dataset && (n > 4) && (strcmp(label[4], "ReferenceState_t") == 0) ) {
                        cgns_state *state = cgi_get_state(cg, B, Z, index[1], index[2], index[3]);
                        if (n == 4)
                            posit = (void *)state;
                        else if (state && (n > 4) && (strcmp(label[4], "DataArray_t") == 0))
                            posit = cgi_get_darray_posit(state->array, state->narrays, index, n, 4);
                        else if (state && (n > 4) && (strcmp(label[4], "UserDefinedData_t") == 0) )
                            posit = cgi_get_user_data_posit(state->user_data, state->nuser_data,
                                index, n, 4);

                    } else if (dataset && (n > 4) && (strcmp(label[4], "UserDefinedData_t") == 0) )
                        posit = cgi_get_user_data_posit(dataset->user_data, dataset->nuser_data,
                            index, n, 4);

                } else if (boco && (n > 3) && (strcmp(label[3], "ReferenceState_t") == 0) ) {
                    cgns_state *state = cgi_get_state(cg, B, Z, index[1], index[2], 0);
                    if (n == 4)
                        posit = (void *)state;
                    else if (state && (n > 4) && (strcmp(label[4], "DataArray_t") == 0))
                        posit = cgi_get_darray_posit(state->array, state->narrays, index, n, 4);
                    else if (state && (n > 4) && (strcmp(label[4], "UserDefinedData_t") == 0) )
                        posit = cgi_get_user_data_posit(state->user_data, state->nuser_data, index, n, 4);

                } else if (boco && (n > 3) && (strcmp(label[3], "BCProperty_t") == 0) ) {
                    cgns_bprop *bprop = cgi_get_bprop(cg, B, Z, index[2]);
                    if (n == 4)
                        posit = (void *) bprop;
                    else if (bprop && (n > 4) && (strcmp(label[4], "WallFunction_t") == 0) ) {
                        cgns_bcwall *bcwall = bprop -> bcwall;
                        if (n == 5)
                            posit = (void *) bcwall;
                        else if (bcwall && (n > 5) && (strcmp(label[5], "UserDefinedData_t") == 0) )
                            posit = cgi_get_user_data_posit(bcwall->user_data, bcwall->nuser_data, index, n, 5);
                    } else if (bprop && (n > 4) && (strcmp(label[4], "Area_t") == 0) ) {
                        cgns_bcarea *bcarea = bprop -> bcarea;
                        if (n == 5)
                            posit = (void *) bcarea;
                        else if (bcarea && (n > 5) && (strcmp(label[5], "UserDefinedData_t") == 0) )
                            posit = cgi_get_user_data_posit(bcarea->user_data, bcarea->nuser_data, index, n, 5);
                        else if (bcarea && (n > 5) && (strcmp(label[5], "DataArray_t") == 0))
                            posit = cgi_get_darray_posit(bcarea->array, bcarea->narrays, index, n, 5);
                    } else if (bprop && (n > 4) && (strcmp(label[4], "UserDefinedData_t") == 0) )
                        posit = cgi_get_user_data_posit(bprop->user_data, bprop->nuser_data, index, n, 4);

                } else if (boco && (n > 3) && (strcmp(label[3], "UserDefinedData_t") == 0) )
                    posit = cgi_get_user_data_posit(boco->user_data, boco->nuser_data, index, n, 3);

            } else if (zboco && (n > 2) && (strcmp(label[2], "ReferenceState_t") == 0) ) {
                cgns_state *state = cgi_get_state(cg, B, Z, index[1], 0, 0);
                if (n == 3)
                    posit = (void *)state;
                else if (state && (n > 3) && (strcmp(label[3], "DataArray_t") == 0))
                    posit = cgi_get_darray_posit(state->array, state->narrays, index, n, 3);
                else if (state && (n > 3) && (strcmp(label[3], "UserDefinedData_t") == 0) )
                    posit = cgi_get_user_data_posit(state->user_data, state->nuser_data, index, n, 3);

            } else if (zboco && (n > 2) && (strcmp(label[2], "UserDefinedData_t") == 0) )
                posit = cgi_get_user_data_posit(zboco->user_data, zboco->nuser_data, index, n, 2);

        } else if ((n > 1) && (strcmp(label[1], "DiscreteData_t") == 0)) {
            cgns_discrete *discrete = cgi_get_discrete(cg, B, Z, index[1]);
            if (n == 2)
                posit = (void *)discrete;
            else if (discrete && (n > 2) && (strcmp(label[2], "DataArray_t") == 0) )
                posit = cgi_get_darray_posit(discrete->array, discrete->narrays, index, n, 2);
            else if (discrete && (n > 2) && (strcmp(label[2], "UserDefinedData_t") == 0) )
                posit = cgi_get_user_data_posit(discrete->user_data, discrete->nuser_data, index, n, 2);

        } else if ((n > 1) && (strcmp(label[1], "FlowEquationSet_t") == 0)) {
            if (n == 2) {
                cgns_equations *equations = cgi_get_equations(cg, B, Z);
                posit = (void *)equations;

            } else if ((n > 2) && (strcmp(label[2], "GoverningEquations_t") == 0) ) {
                cgns_governing *governing =  cgi_get_governing(cg, B, Z);

                if (n == 3)
                    posit = (void *)governing;
                else if (governing && (n > 3) && (strcmp(label[3], "UserDefinedData_t") == 0) )
                    posit = cgi_get_user_data_posit(governing->user_data, governing->nuser_data,
                        index, n, 3);

            } else if (n > 2 && (strcmp(label[2], "GasModel_t") == 0
                || strcmp(label[2], "ViscosityModel_t") == 0
                || strcmp(label[2], "ThermalConductivityModel_t") == 0
                || strcmp(label[2], "TurbulenceModel_t") == 0
                || strcmp(label[2], "TurbulenceClosure_t") == 0
                || strcmp(label[2], "ThermalRelaxationModel_t") == 0
                || strcmp(label[2], "ChemicalKineticsModel_t") == 0)) {
                cgns_model *model = cgi_get_model(cg, B, Z, label[2]);
                if (n == 3)
                    posit = (void *)model;
                else if (model && (n > 3) && (strcmp(label[3], "DataArray_t") == 0))
                    posit = cgi_get_darray_posit(model->array, model->narrays, index, n, 3);
                else if (model && (n > 3) && (strcmp(label[3], "UserDefinedData_t") == 0) )
                    posit = cgi_get_user_data_posit(model->user_data, model->nuser_data, index, n, 3);

            } else if ((n > 2) && (strcmp(label[2], "UserDefinedData_t") == 0)) {
                cgns_equations *equations = cgi_get_equations(cg, B, Z);
                if (equations)
                    posit = cgi_get_user_data_posit(equations->user_data, equations->nuser_data,
                        index, n, 2);
            }

        } else if ((n > 1) && (strcmp(label[1], "ConvergenceHistory_t") == 0)) {
            cgns_converg *converg = cgi_get_converg(cg, B, Z);
            if (n == 2)
                posit = (void *)converg;
            else if (converg && (n > 2) && (strcmp(label[2], "DataArray_t") == 0) )
                posit = cgi_get_darray_posit(converg->array, converg->narrays, index, n, 2);
            else if (converg && (n > 2) && (strcmp(label[2], "UserDefinedData_t") == 0) )
                posit = cgi_get_user_data_posit(converg->user_data, converg->nuser_data, index, n, 2);

        } else if ((n > 1) && (strcmp(label[1], "IntegralData_t") == 0)) {
            cgns_integral *integral = cgi_get_integral(cg, B, Z, index[1]);
            if (n == 2)
                posit = (void *)integral;
            else if (integral && (n > 2) && (strcmp(label[2], "DataArray_t") == 0) )
                posit = cgi_get_darray_posit(integral->array, integral->narrays, index, n, 2);
            else if (integral && (n > 2) && (strcmp(label[2], "UserDefinedData_t") == 0) )
                posit = cgi_get_user_data_posit(integral->user_data, integral->nuser_data, index, n, 2);

        } else if ((n > 1) && (strcmp(label[1], "ReferenceState_t") == 0)) {
            cgns_state *state = cgi_get_state(cg, B, Z, 0, 0, 0);
            if (n == 2)
                posit = (void *)state;
            else if (state && (n > 2) && (strcmp(label[2], "DataArray_t") == 0))
                posit = cgi_get_darray_posit(state->array, state->narrays, index, n, 2);
            else if (state && (n > 2) && (strcmp(label[2], "UserDefinedData_t") == 0) )
                posit = cgi_get_user_data_posit(state->user_data, state->nuser_data, index, n, 2);

        } else if ((n > 1) && (strcmp(label[1], "UserDefinedData_t") == 0)) {
            cgns_zone *zone = cgi_get_zone(cg, B, Z);
            if (zone)
                posit = cgi_get_user_data_posit(zone->user_data, zone->nuser_data, index, n, 1);

        } else if ((n > 1) && (strcmp(label[1], "RotatingCoordinates_t") == 0)) {
            cgns_rotating *rotating = cgi_get_rotating(cg, B, Z);
            if (n == 2)
                posit = (void *)rotating;
            else if (rotating && (n > 2) && (strcmp(label[2], "DataArray_t") == 0))
                posit = cgi_get_darray_posit(rotating->array, rotating->narrays, index, n, 2);
            else if (rotating && (n > 2) && (strcmp(label[2], "UserDefinedData_t") == 0) )
                posit = cgi_get_user_data_posit(rotating->user_data, rotating->nuser_data, index, n, 2);
        }
    /* Process children of the base node other than Zone_t. */
    } else if ((n > 0) && (strcmp(label[0], "ReferenceState_t") == 0)) {
        cgns_state *state = cgi_get_state(cg, B, 0, 0, 0, 0);
        if (n == 1)
            posit = (void *)state;
        else if (state && (n > 1) && (strcmp(label[1], "DataArray_t") == 0))
            posit = cgi_get_darray_posit(state->array, state->narrays, index, n, 1);
        else if (state && (n > 1) && (strcmp(label[1], "UserDefinedData_t") == 0) )
            posit = cgi_get_user_data_posit(state->user_data, state->nuser_data, index, n, 1);

    } else if ((n > 0) && (strcmp(label[0], "Family_t") == 0)) {
        cgns_family *family = cgi_get_family(cg, B, index[0]);
        if (n == 1) {
            posit = (void *)family;
        } else if (family && (n > 1) && (strcmp(label[1], "GeometryReference_t") == 0) ) {
            cgns_geo *geo = 0;

            /* check index */
            if (family && index[1] <= family->ngeos && index[1] > 0)
                geo = &family->geo[index[1]-1];
            else
                cgi_error("GeometryReference_t node number %d invalid",index[1]);

            if (n == 2)
                posit = (void *)geo;
            else if (geo && (n > 2) && (strcmp(label[2], "UserDefinedData_t") == 0) )
                posit = cgi_get_user_data_posit(geo->user_data, geo->nuser_data, index, n, 2);

        } else if (family && (n > 1) && (strcmp(label[1], "UserDefinedData_t") == 0) ) {
            posit = cgi_get_user_data_posit(family->user_data, family->nuser_data, index, n, 1);
        }

    } else if ((n > 0) && (strcmp(label[0], "BaseIterativeData_t") == 0)) {
        cgns_biter *biter = cgi_get_biter(cg, B);
        if (n == 1)
            posit = (void *)biter;
        else if (biter && (n > 1) && (strcmp(label[1], "DataArray_t") == 0) )
            posit = cgi_get_darray_posit(biter->array, biter->narrays, index, n, 1);
        else if (biter && (n > 1) && (strcmp(label[1], "UserDefinedData_t") == 0) )
            posit = cgi_get_user_data_posit(biter->user_data, biter->nuser_data, index, n, 1);

    } else if ((n > 0) && (strcmp(label[0], "ConvergenceHistory_t") == 0)) {
        cgns_converg *converg = cgi_get_converg(cg, B, 0);
        if (n == 1)
            posit = (void *)converg;
        else if (converg && (n > 1) && (strcmp(label[1], "DataArray_t") == 0) )
            posit = cgi_get_darray_posit(converg->array, converg->narrays, index, n, 1);
        else if (converg && (n > 1) && (strcmp(label[1], "UserDefinedData_t") == 0) )
            posit = cgi_get_user_data_posit(converg->user_data, converg->nuser_data, index, n, 1);

    } else if ((n > 0) && (strcmp(label[0], "FlowEquationSet_t") == 0)) {
        if (n == 1) {
            cgns_equations *equations = cgi_get_equations(cg, B, 0);
            posit = (void *)equations;
        } else if ((n > 1) && (strcmp(label[1], "GoverningEquations_t") == 0) ) {
            cgns_governing *governing =  cgi_get_governing(cg, B, 0);

            if (n == 2)
                posit = (void *)governing;
            else if (governing && (n > 2) && (strcmp(label[2], "UserDefinedData_t") == 0) )
                posit = cgi_get_user_data_posit(governing->user_data, governing->nuser_data, index, n, 2);

        } else if (n > 1 && (strcmp(label[1], "GasModel_t") == 0
            || strcmp(label[1], "ViscosityModel_t") == 0
            || strcmp(label[1], "ThermalConductivityModel_t") == 0
            || strcmp(label[1], "TurbulenceModel_t") == 0
            || strcmp(label[1], "TurbulenceClosure_t") == 0
            || strcmp(label[1], "ThermalRelaxationModel_t") == 0
            || strcmp(label[1], "ChemicalKineticsModel_t") == 0)) {
            cgns_model *model = cgi_get_model(cg, B, 0, label[1]);

            if (n == 2) posit = (void *)model;

            else if (model && (n > 2) && (strcmp(label[2], "DataArray_t") == 0))
                posit = cgi_get_darray_posit(model->array, model->narrays, index, n, 2);
            else if (model && (n > 2) && (strcmp(label[2], "UserDefinedData_t") == 0) )
                posit = cgi_get_user_data_posit(model->user_data, model->nuser_data, index, n, 2);

        } else if ((n > 1) && (strcmp(label[1], "UserDefinedData_t") == 0)) {
            cgns_equations *equations = cgi_get_equations(cg, B, 0);
            if (equations)
                posit = cgi_get_user_data_posit(equations->user_data, equations->nuser_data, index, n, 1);
        }

    } else if ((n > 0) && (strcmp(label[0], "IntegralData_t") == 0)) {
        cgns_integral *integral = cgi_get_integral(cg, B, 0, index[0]);
        if (n == 1)
            posit = (void *)integral;
        else if (integral && (n > 1) && (strcmp(label[1], "DataArray_t") == 0) )
            posit = cgi_get_darray_posit(integral->array, integral->narrays, index, n, 1);
        else if (integral && (n > 1) && (strcmp(label[1], "UserDefinedData_t") == 0) )
            posit = cgi_get_user_data_posit(integral->user_data, integral->nuser_data, index, n, 1);

    } else if ((n > 0) && (strcmp(label[0], "UserDefinedData_t") == 0)) {
        cgns_base *base = cgi_get_base(cg, B);
        if (base)
            posit = cgi_get_user_data_posit(base->user_data, base->nuser_data, index, n, 0);

    } else if ((n > 0) && (strcmp(label[0], "Gravity_t") == 0)) {
        cgns_gravity *gravity = cgi_get_gravity(cg, B);
        if (n == 1)
            posit = (void *)gravity;
        else if (gravity && (n > 1) && (strcmp(label[1], "DataArray_t") == 0) )
            posit = cgi_get_darray_posit(gravity->vector, gravity->narrays, index, n, 1);
        else if (gravity && (n > 1) && (strcmp(label[1], "UserDefinedData_t") == 0) )
            posit = cgi_get_user_data_posit(gravity->user_data, gravity->nuser_data, index, n, 1);

    } else if ((n > 0) && (strcmp(label[0], "Axisymmetry_t") == 0)) {
        cgns_axisym *axisym = cgi_get_axisym(cg, B);
        if (n == 1)
            posit = (void *)axisym;
        else if (axisym && (n > 1) && (strcmp(label[1], "DataArray_t") == 0) )
            posit = cgi_get_darray_posit(axisym->array, axisym->narrays, index, n, 1);
        else if (axisym && (n > 1) && (strcmp(label[1], "UserDefinedData_t") == 0) )
            posit = cgi_get_user_data_posit(axisym->user_data, axisym->nuser_data, index, n, 1);

    } else if ((n > 0) && (strcmp(label[0], "RotatingCoordinates_t") == 0)) {
        cgns_rotating *rotating = cgi_get_rotating(cg, B, 0);
        if (n == 1)
            posit = (void *)rotating;
        else if (rotating && (n > 1) && (strcmp(label[1], "DataArray_t") == 0))
            posit = cgi_get_darray_posit(rotating->array, rotating->narrays, index, n, 1);
        else if (rotating && (n > 1) && (strcmp(label[1], "UserDefinedData_t") == 0))
            posit = cgi_get_user_data_posit(rotating->user_data, rotating->nuser_data, index, n, 1);

    } else {
        cgi_error("The path prescribed in the 'cg_goto' function is not valid.");
        (*ier)=CG_INCORRECT_PATH;
        return 0;
    }

    if (posit==0) {
        cgi_error("The path specified in function 'cg_goto' lead to a non-existant node");
...
/* This function returns the ADF ID number (ADF address) of the ADF node   *\
|* pointed to by the arguments of the cg_goto function.  Every ADF nodes   *|
\* that can be reached with the cg_goto function must be represented here. */
    ...
    if (strcmp(posit_label,"CGNSBase_t")==0) {
        ...
        (*posit_id) = rotating->id;

    } else if (strcmp(posit_label,"BCProperty_t")==0) {
        cgns_bprop *bprop = (cgns_bprop *) posit;
        (*posit_id) = bprop->id;

    } else if (strcmp(posit_label,"WallFunction_t")==0) {
        cgns_bcwall *bcwall = (cgns_bcwall *) posit;
        (*posit_id) = bcwall->id;
        ...
        cgns_cprop *cprop = (cgns_cprop *) posit;
        (*posit_id) = cprop->id;
        ...
        cgns_caverage *caverage = (cgns_caverage *) posit;
        (*posit_id) = caverage->id;

    } else {
        cgi_error("Can't find the ADF ID of node %s",posit_label);
        return 1;
    }
 ...
 *  Zone_t, BC_t
 */
    if (strcmp(posit_label,"Zone_t")==0) {
        ...
        cgi_error("FamilyName_t node not supported under '%s' type node",posit_label);
        (*ier) = CG_INCORRECT_PATH;
        return 0;
    }
    if (cg->mode == MODE_MODIFY && local_mode == MODE_WRITE) {
 ...
 */
    if (strcmp(posit_label,"CGNSBase_t")==0)
        ...
        cgi_error("DataClass_t node not supported under '%s' type node",posit_label);
        (*ier) = CG_INCORRECT_PATH;
        return 0;
    }
    if (cg->mode == MODE_MODIFY && local_mode == MODE_WRITE) {
 ...
 */
    if (strcmp(posit_label,"CGNSBase_t")==0)
        ...
        cgi_error("DimensionalUnits_t node not supported under '%s' type node",posit_label);
        (*ier) = CG_INCORRECT_PATH;
        return 0;
    }
    if (error1==1) {
        cgi_error("DimensionalUnits_t already defined under %s",posit_label);
        (*ier) = CG_ERROR;
        return 0;
    }
    if (!units && local_mode == MODE_READ) {
        cgi_error("DimensionalUnits_t Node doesn't exist under %s",posit_label);
        (*ier) = CG_NODE_NOT_FOUND;
        return 0;
    }
    if (parent_id) {
 ...
 */
    if (strcmp(posit_label,"Zone_t")==0)
        ...
        cgi_error("Ordinal_t node not supported under '%s' type node",posit_label);
        (*ier) = CG_INCORRECT_PATH;
        return 0;
    }
    if (cg->mode == MODE_MODIFY && local_mode == MODE_WRITE) {
 ...
 */
    if (strcmp(posit_label,"GridCoordinates_t")==0)
        ...
        cgi_error("Rind_t node not supported under '%s' type node",posit_label);
        (*ier) = CG_INCORRECT_PATH;
        return 0;
    }
...
/* Corrected on July 27 2001 by Diane Poirier
    if (error1==1) {
        cgi_error("Rind_t already defined under %s",posit_label);
        (*ier) = CG_ERROR;
        return 0;
    }
*/
    if (!rind_planes && local_mode == MODE_READ) {
        cgi_error("Rind_t node doesn't exist under %s",posit_label);
        (*ier) = CG_NODE_NOT_FOUND;
        return 0;
    }
    if (parent_id && cg->mode==MODE_MODIFY) {
 ...
 */
    if (strcmp(posit_label,"FlowSolution_t")==0)
        ...
        cgi_error("GridLocation_t node not supported under '%s' type node",posit_label);
        (*ier) = CG_INCORRECT_PATH;
        return 0;
    }
    if (cg->mode == MODE_MODIFY && local_mode == MODE_WRITE) {
 ...
 */
    if (strcmp(posit_label,"DataArray_t")==0)
        ADDRESS4SINGLE(cgns_array,convert, cgns_conversion, 1)

    else {
        cgi_error("DataConversion_t node not supported under '%s' type node",posit_label);
        (*ier) = CG_INCORRECT_PATH;
        return 0;
    }
    if (error1==1) {
        cgi_error("DataConversion_t already defined under %s",posit_label);
        (*ier) = CG_ERROR;
        return 0;
    }
    if (!convert && local_mode == MODE_READ) {
        cgi_error("DataConversion_t node does not exist under %s",posit_label);
        (*ier) = CG_NODE_NOT_FOUND;
        return 0;
    }
    if (parent_id) {
 ...
 */
    if (strcmp(posit_label,"DataArray_t")==0)
        ...
        cgi_error("DimensionalExponents_t node not supported under '%s' type node",posit_label);
        (*ier) = CG_INCORRECT_PATH;
        return 0;
    }
    if (error1==1) {
        cgi_error("DimensionalExponents_t already defined under %s",posit_label);
        (*ier) = CG_ERROR;
        return 0;
    }
    if (!exponents && local_mode == MODE_READ) {
        cgi_error("DimensionalExponents_t node does not exist under %s",posit_label);
        (*ier) = CG_NODE_NOT_FOUND;
        return 0;
    }
    if (parent_id) {
 ...
 */
    if (strcmp(posit_label,"CGNSBase_t")==0)
        ...
        cgi_error("FlowEquationSet_t node not supported under '%s' type node",posit_label);
        (*ier) = CG_INCORRECT_PATH;
        return 0;
    }
    if (error1==1) {
        cgi_error("FlowEquationSet_t already defined under %s",posit_label);
        (*ier) = CG_ERROR;
        return 0;
    }
    if (!equations && local_mode == MODE_READ) {
        cgi_error("FlowEquationSet_t Node doesn't exist under %s",posit_label);
        (*ier) = CG_NODE_NOT_FOUND;
        return 0;
    }
    if (parent_id) {
 ...
 */
    if (strcmp(posit_label,"CGNSBase_t")==0)
        ...
        cgi_error("ReferenceState_t node not supported under '%s' type node",posit_label);
        (*ier) = CG_INCORRECT_PATH;
        return 0;
    }
    if (error1==1) {
        cgi_error("ReferenceState_t already defined under %s",posit_label);
        (*ier) = CG_ERROR;
        return 0;
    }
    if (!state && local_mode == MODE_READ) {
        cgi_error("ReferenceState_t Node doesn't exist under %s",posit_label);
        (*ier) = CG_NODE_NOT_FOUND;
        return 0;
    }
    if (parent_id) {
 ...
 */
    if (strcmp(posit_label,"CGNSBase_t")==0) {
        ...
        cgi_error("ConvergenceHistory_t node not supported under '%s' type node",posit_label);
        (*ier) = CG_INCORRECT_PATH;
        return 0;
    }
    if (error1==1) {
        cgi_error("ConvergenceHistory_t already defined under %s",posit_label);
        (*ier) = CG_ERROR;
        return 0;
    }
    if (!converg && local_mode == MODE_READ) {
        cgi_error("ConvergenceHistory_t Node doesn't exist under %s",posit_label);
        (*ier) = CG_NODE_NOT_FOUND;
        return 0;
    }
    if (parent_id) {
...
/* Possible parents for GoverningEquations_t:
    FlowEquationSet_t
 */
    if (strcmp(posit_label,"FlowEquationSet_t")==0)
        ADDRESS4SINGLE(cgns_equations, governing, cgns_governing,1)
    else {
        cgi_error("GoverningEquations_t node not supported under '%s' type node",posit_label);
        (*ier) = CG_INCORRECT_PATH;
        return 0;
    }
    if (error1==1) {
        cgi_error("GoverningEquations_t already defined under %s",posit_label);
        (*ier) = CG_ERROR;
        return 0;
    }
    if (!governing && local_mode == MODE_READ) {
        cgi_error("ConvergenceHistory_t Node doesn't exist under %s",posit_label);
        (*ier) = CG_NODE_NOT_FOUND;
        return 0;
    }
 ...
 */
    if (strcmp(posit_label,"GoverningEquations_t")==0)
        ADDRESS4SINGLE(cgns_governing, diffusion_model, int, 6)

    else if (strcmp(posit_label,"TurbulenceModel_t")==0)
        ADDRESS4SINGLE(cgns_model, diffusion_model, int, 6)

    else {
        cgi_error("Diffusion Model node not supported under '%s' type node",posit_label);
        (*ier) = CG_INCORRECT_PATH;
        return 0;
    }
    if (error1==1) {
        cgi_error("Diffusion Model already defined under %s",posit_label);
        (*ier) = CG_ERROR;
        return 0;
    }
    if (!diffusion_model && local_mode == MODE_READ) {
        cgi_error("Diffusion Model Node doesn't exist under %s",posit_label);
        (*ier) = CG_NODE_NOT_FOUND;
        return 0;
    }
    if (parent_id) {
 ...
 */
    ...
    if (strcmp(posit_label,"GridCoordinates_t")==0) {
     ...
     /* 2 DataArray_t under Elements_t: connect and parent */
        ...
        cgns_section *section= (cgns_section *)posit;
        if (local_mode==MODE_WRITE) {
            ...
            if (section->connect==0 && strcmp(given_name,"ElementConnectivity")==0) {
                ...
                if (cg->mode == MODE_WRITE) error1=1;
                else {
     ...
     /* 0,1 DataArray_t under GridConnectivity_t */
        ...
        if (local_mode==MODE_WRITE && strcmp(given_name,"InterpolantsDonor")) {
            cgi_error("User defined DataArray_t node not supported under '%s' type node",posit_label);
            (*ier) = CG_ERROR;
            return 0;
        }
     ...
     /* 0,1 DataArray_t for GravityVector */
        ...
        cgns_array *vector;
        if (local_mode==MODE_WRITE && strcmp(given_name,"GravityVector")) {
            cgi_error("User defined DataArray_t node not supported under '%s' type node",posit_label);
            (*ier) = CG_ERROR;
            return 0;
        }
     ...
     /* 2,4 DataArray_t for Axisymmetry_t */
        ...
        if (local_mode==MODE_WRITE && strcmp(given_name,"AxisymmetryReferencePoint") &&
     ...
     /* 2 DataArray_t for RotatingCoordinates_t */
        ...
        if (local_mode==MODE_WRITE && strcmp(given_name,"RotationCenter") &&
     ...
     /* 2 DataArray_t for Area_t:  SurfaceArea, RegionName */
        ...
        if (local_mode==MODE_WRITE && strcmp(given_name,"SurfaceArea") &&
     ...
     /* 3 DataArray_t for Periodic_t: RotationCenter, RotationAngle, Translation */
        ...
        cgi_error("DataArray_t node not supported under '%s' type node",posit_label);
        (*ier) = CG_INCORRECT_PATH;
        return 0;
    }
    if (error1) {
 ...
 */
    if (strcmp(posit_label,"FlowEquationSet_t")==0) {
        ...
        cgi_error("%s node not supported under '%s' type node",ModelLabel,posit_label);
        (*ier)=CG_INCORRECT_PATH;
        return 0;
    }
    if (!model && local_mode == MODE_READ) {
        cgi_error("%s node doesn't exist under %s",ModelLabel,posit_label);
        (*ier) = CG_NODE_NOT_FOUND;
        return 0;
    }
    if (error1) {
        cgi_error("%s node already defined under %s",ModelLabel,posit_label);
        (*ier) = CG_ERROR;
        return 0;
    }
    if (parent_id) {
 ...
 */
    if (strcmp(posit_label,"IntegralData_t")==0)
        ...
        || (strcmp(posit_label,"ThermalConductivityModel_t")==0)
        || (strcmp(posit_label,"TurbulenceModel_t")==0)
        || (strcmp(posit_label,"TurbulenceClosureModel_t")==0)
        || (strcmp(posit_label,"ThermalRelaxationModel_t")==0)
        || (strcmp(posit_label,"ChemicalKineticsModel_t")==0) )
 ...
 */
    if (strcmp(posit_label,"CGNSBase_t")==0)
        ...
        cgi_error("RotatingCoordinates_t node not supported under '%s' type node",posit_label);
        (*ier) = CG_INCORRECT_PATH;
        return 0;
    }
    if (error1==1) {
        cgi_error("RotatingCoordinates_t already defined under %s",posit_label);
        (*ier) = CG_ERROR;
        return 0;
    }
    if (!rotating && local_mode == MODE_READ) {
        cgi_error("RotatingCoordinates_t Node doesn't exist under %s",posit_label);
        (*ier) = CG_NODE_NOT_FOUND;
        return 0;
    }
    if (parent_id) {
...
void cgi_free_boco(cgns_boco *boco) {
    ...
    if (boco->ptset) cgi_free_ptset(boco->ptset);
...
int cgi_MassUnits(char *Name, MassUnits_t *mass_unit) {
    ...
    for (i=0; i<NofValidMassUnits; i++) {
        if (strcmp(mass, MassUnitsName[i])==0) {
...
int cgi_LengthUnits(char *Name, LengthUnits_t *length_unit) {
    ...
    for (i=0; i<NofValidLengthUnits; i++) {
        if (strcmp(length, LengthUnitsName[i])==0) {
...
int cgi_TimeUnits(char *Name, TimeUnits_t *time_unit) {
    ...
    for (i=0; i<NofValidTimeUnits; i++) {
        if (strcmp(time, TimeUnitsName[i])==0) {
...
int cgi_TemperatureUnits(char *Name, TemperatureUnits_t *temperature_unit) {
    ...
    for (i=0; i<NofValidTemperatureUnits; i++) {
        if (strcmp(temp, TemperatureUnitsName[i])==0) {
...
int cgi_AngleUnits(char *Name, AngleUnits_t *angle_unit) {
    ...
    for (i=0; i<NofValidAngleUnits; i++) {
        if (strcmp(angle, AngleUnitsName[i])==0) {
